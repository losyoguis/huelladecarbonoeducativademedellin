...
ok(/data-section="seccion-5"[^>]*>[\s\S]*?<span>5<\/span>[\s\S]*?<strong>Estado de Información<\/strong>/.test(html),'Falta sección 5 Estado información');
ok(/data-section="seccion-6"[^>]*>[\s\S]*?<span>6<\/span>[\s\S]*?<strong>Aula<\/strong>/.test(html),'Aula no es sección 5');
...
ok(html.includes('app.js?v=100-ux-organizada'),'Falta cache-busting v97');
