...
ok(html.includes('data/registros.electricidad.min.js?v=105-inem-electricidad" as="script" fetchpriority="high"'),'Bundle eléctrico no tiene prioridad alta');
ok(html.includes('data/resumenes.electricidad.min.js?v=105-inem-electricidad" as="script" fetchpriority="high"'),'Resumen eléctrico no tiene prioridad alta');
ok(html.includes('<script src="data/registros.electricidad.min.js?v=105-inem-electricidad" fetchpriority="high"></script>'),'Script eléctrico no tiene prioridad alta');
ok(html.includes('<script defer src="app.js?v=105-inem-electricidad" fetchpriority="high"></script>'),'app.js no tiene prioridad alta');
for(const file of ['data/registros.agua.min.js','data/registros.gas.min.js','assistant.js']){
  ok(html.includes(`${file}?v=105-inem-electricidad" fetchpriority="low"`),`${file} no tiene prioridad baja`);
}
