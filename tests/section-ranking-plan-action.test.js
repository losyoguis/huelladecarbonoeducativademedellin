...
ok(/data-section="seccion-3"[^>]*>[\s\S]*?<span>3<\/span>[\s\S]*?<strong>Ranking<\/strong>/.test(html),'Falta sección 3 Ranking');
ok(/data-section="seccion-6"[^>]*>[\s\S]*?<span>6<\/span>[\s\S]*?<strong>Aula<\/strong>/.test(html),'Aula no es sección 6');
...
ok(html.includes('app.js?v=98-sidebar-legible'),'Falta cache-busting JS v97');
