...
ok(app.includes("const DATA_VERSION = 'v104-videotutorial-segundo-cero-20260811';"),'DATA_VERSION no es v104');
ok(html.includes('app.js?v=104-video-segundo-0'),'Cache-busting v104 ausente');
...
ok(html.includes('start=0'),'El videotutorial no inicia desde el segundo 0');
ok(!/start=\x31\x38/.test(html)&&!new RegExp('segundo\\s+'+(9*2)).test(html),'Persisten referencias de inicio antiguas');
console.log(JSON.stringify({ok:true,sections:9,menu:'top-cards',cards:10,electricity:6,water:7,gas:8,invoices:9,tutorial:10,tutorialStart:0,firstVisitAuto:true,googleSitesResponsive:true},null,2));
