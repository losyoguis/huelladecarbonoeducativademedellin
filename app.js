/* SiMeCO₂ v98 · sidebar lateral legible y elegante para Google Sites */
...
const DATA_VERSION = 'v98-sidebar-legible-google-sites-20260808';
...
function extractGasM3(text){
  const segment=metricSegment(String(text||''),'Gas',['Energ[ií]a','Alcantarillado','(?:Agua|Acueducto)']);
  return lastMeasurementMatch(segment,[
    /Consumo\s+[A-Za-z]{3,4}-\d{2}\s+([\d.,]+)\s*x/gi,
    /([\d.,]+)\s*(?:m3|mt\s*3|m³)[\s\S]{0,80}?=\s*Consumo/gi
  ],'gas');
}
function nthMeasurementByLabel(text,label,n,unit,kind='generic'){
  const re=new RegExp(label+'\\s+([\\d.,]+)\\s*'+unit,'gi');
  let match,index=0;
  while((match=re.exec(text))){
    index++;
    if(index===n) return parseMeasurement(match[1],kind);
  }
  return null;
}
...
function hasAnyMeasure(r){ return recordHasEnergyReading(r) || (r?.waterM3!==null && r?.waterM3!==undefined && r?.waterM3!=='' && Number.isFinite(Number(r.waterM3))) || (r?.gasM3!==null && r?.gasM3!==undefined && r?.gasM3!=='' && Number.isFinite(Number(r.gasM3))); }
...
function bindTerritoryFilters(){
  const configs=[
    ['compare','compareTerritoryType','compareTerritory','compareNucleus',()=>{ if($('compareSite')) $('compareSite').value=''; const f=siteAutocompleteState.get('compareSiteSearch'); if(f){f.input.value='';f.clear.classList.remove('visible');} refreshHistoryModule({preserveSelection:true}); }],
...
function chooseSiteFieldSuggestion(item,key){
  ...
  if(item.mode==='compare'){
    ...
    refreshHistoryModule({preserveSelection:true});
  }else{
...
function bindEvents(){
  ...
  bindEvent('compareBtn','click',()=>refreshHistoryModule({preserveSelection:true}));
...
function parseSummary(text){
  ...
  return {
    ...
    gasM3: nthMeasurementByLabel(head,'ACTUAL',3,'m3','gas'),
    gasValue: valueAfter(head,'Total Gas Natural') ?? valueAfter(head,'Total Gas'),
...
function parseBlock(block, period, fileName, sourceUrl, idx){
  ...
  return {
    ...
    gasM3,
    gasValue:valueAfter(text,'Total Gas Natural') ?? valueAfter(text,'Total Gas'),
    energyKwh,
...
function renderCompareControls(opts={}){
  ...
  if(groups.length){
    ...
    $('compareA').value=validKeys.has(currentA)?currentA:groups[0].key;
    $('compareB').value=validKeys.has(currentB)?currentB:groups[groups.length-1].key;
    // Evita seleccionar el mismo periodo cuando existen al menos dos.
    if(groups.length>1 && $('compareA').value===$('compareB').value){
      $('compareA').value=groups[0].key;
      $('compareB').value=groups[groups.length-1].key;
    }
  }
  syncSearchableSelect('compareA');
  syncSearchableSelect('compareB');
}
...
function refreshHistoryModule(options={}){
  try{
    ...
    if($('compareResult')) $('compareResult').innerHTML='<p class="bad">No fue posible actualizar el histórico. Recarga la página o limpia los filtros e inténtalo nuevamente.</p>';
    return false;
  }
}
window.simecoRefreshHistory=refreshHistoryModule;
...
function drawChart(data){
  ...
  const containerWidth=Math.max(320,Math.floor(c.parentElement?.clientWidth||c.clientWidth||1100));
  const canvasWidth=Math.max(containerWidth,Math.max(cleanData.length,1)*92+170),canvasHeight=470;
...
function openPdfPrintDocument(title, subtitle, content){
  ...
  const css=`
    ...
    .gas-plan-document{box-shadow:none!important;border:0!important;padding:0!important}.gas-plan-header{display:flex;justify-content:space-between;gap:16px;padding:16px;border-radius:14px;background:#a85a00;color:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact}.gas-plan-header h3{margin:4px 0;color:#fff}.gas-plan-header p{margin:3px 0}.gas-plan-priority{padding:9px 11px;border:1px solid rgba(255,255,255,.45);border-radius:10px;text-align:center}.gas-plan-priority span,.gas-plan-priority strong{display:block}.gas-plan-metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:12px 0}.gas-plan-metrics article{padding:10px;border:1px solid #f1d7b5;border-radius:10px;background:#fff8ef}.gas-plan-metrics span{display:block;font-size:8pt;color:#75644f}.gas-plan-metrics strong{display:block;margin-top:3px;color:#8a4900}.gas-plan-goal,.gas-plan-followup{padding:11px 13px;border-left:5px solid #e89422;background:#fff5e7;border-radius:9px}.gas-plan-document h4{color:#884800;margin:16px 0 7px}.plan-history-chart{margin:8px 0 12px;padding:8px;border:1px solid #d8e8e5;border-radius:12px;page-break-inside:avoid}.plan-history-svg{display:block;width:100%;height:auto;max-height:330px}.plan-history-chart-empty{padding:16px;text-align:center;color:#687d78;background:#f6faf9;border-radius:10px}
...
function downloadDashboardPdfReport(){
  ...
  const rows=agg.map((x,i)=>`<tr><td>${i+1}</td><td>${escapeHtml(x.displaySite||x.site)}</td><td>${mapAddressLink(x.address,x.address||'Sin dirección',x.displaySite||x.site)}</td><td>${x.periodCount}</td><td>${x.hasEnergy?`${fmt(x.energyKwh)} kWh`:'N.I.'}</td><td>${x.hasEnergy?`${fmt(x.co2kg/1000)} t`:'No calculable'}</td><td>${x.hasEnergy?fmt(x.trees):'No calculable'}</td><td>${escapeHtml(qualityStatusForSite(x).label)}</td></tr>`).join('');
...
function renderDashboard(){
  ...
  const body = rows.map((r,i)=>{
    ...
    const priority = r.hasEnergy ? classifyEnergyIntensity(r.avgKwhMonth) : (energyState.code==='external'?{level:'Energía en contrato separado',short:'Contrato separado',cls:'external'}:{level:'Energía no identificada',short:'N.I.',cls:'pending'});
    const energyText=r.hasEnergy?`<strong>${fmt(r.energyKwh)}</strong>`:(energyState.code==='external'?'<span class="quality-badge external">Contrato separado</span>':'<span class="not-available ni-abbr" title="No identificada">N.I.</span>');
    ...
    const avgText=r.hasEnergy?fmt(r.avgKwhMonth):(energyState.code==='external'?'<span class="not-available">Contrato separado</span>':'<span class="not-available ni-abbr" title="No identificada">N.I.</span>');
