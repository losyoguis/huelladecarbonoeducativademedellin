...
const DATA_VERSION = 'v67-ranking-direccion-20260807';
...
function downloadDashboardPdfReport(){
  ...
  const rows=agg.map((x,i)=>`<tr><td>${i+1}</td><td>${escapeHtml(x.displaySite||x.site)}</td><td>${escapeHtml(x.address||'')}</td><td>${x.periodCount}</td><td>${x.hasEnergy?`${fmt(x.energyKwh)} kWh`:'No identificada'}</td><td>${x.hasEnergy?`${fmt(x.co2kg/1000)} t`:'No calculable'}</td><td>${x.hasEnergy?fmt(x.trees):'No calculable'}</td><td>${escapeHtml(qualityStatusForSite(x).label)}</td></tr>`).join('');
...
function handleRankingSuggestionClick(e){const btn=e.target.closest('[data-ranking-key]');if(!btn)return;e.preventDefault();selectRankingSuggestion(btn.dataset.rankingKey,Number(btn.dataset.rankingIndex));}
...
function drawSiteChart(rows){
  ...
  if($('rankingExtremes'))$('rankingExtremes').innerHTML=total?`${most?`<article><span>Mayor ${escapeHtml(metric.short.toLowerCase())}</span><strong>${escapeHtml(most.displaySite||most.site)}</strong><small>📍 ${escapeHtml(rankingAddressText(most))} · ${fmt(most.rankingValue)} ${metric.unit}</small></article>`:'<article><span>Mayor valor</span><strong>Sin lecturas</strong><small>No hay datos para comparar</small></article>'}${least?`<article><span>Menor valor con dato</span><strong>${escapeHtml(least.displaySite||least.site)}</strong><small>📍 ${escapeHtml(rankingAddressText(least))} · ${fmt(least.rankingValue)} ${metric.unit}</small></article>`:''}<article><span>Cobertura</span><strong>${measured.length} con dato · ${external.length} contrato separado · ${pending.length} pendientes</strong><small>${total} sedes históricas</small></article>`:'<p>No hay sedes históricas para construir el ranking.</p>';
    ...
    <article class="plan-document">
      <div class="plan-cover">
        <div>
          ...
          <p>${escapeHtml(d.address)}</p>${d.invoiceSite&&d.invoiceSite!==d.site?`<p class="plan-invoice-alias">Nombre en factura: ${escapeHtml(d.invoiceSite)}</p>`:''}
        </div>
