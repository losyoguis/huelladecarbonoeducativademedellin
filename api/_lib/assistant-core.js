...
const TOOL_DEFS = [
  ...
  },
  {
    ...
    parameters:{type:'object',properties:{query:{type:'string'},metric:{type:'string',enum:['energyKwh']},year:{type:['string','null']}},required:['query','metric','year'],additionalProperties:false},
    strict:true,
  },
  {
    type:'function', name:'obtener_ranking',
    description:'Obtiene el ranking institucional por consumo de energía eléctrica. Las instituciones sin dato no se convierten en cero y se reportan como excluidas con su estado de calidad.',
    parameters:{type:'object',properties:{metric:{type:'string',enum:['energyKwh']},period:{type:['string','null']},year:{type:['string','null']},limit:{type:'integer',minimum:1,maximum:50}},required:['metric','period','year','limit'],additionalProperties:false},
    strict:true,
  },
  {
    type:'function', name:'obtener_posicion_institucion',
    description:'Indica si una institución entra o no en el ranking de energía eléctrica, su posición cuando aplica y la razón exacta cuando queda excluida. Úsala para preguntas como por qué una I.E. no aparece en el ranking.',
    parameters:{type:'object',properties:{query:{type:'string'},metric:{type:'string',enum:['energyKwh']},period:{type:['string','null']},year:{type:['string','null']}},required:['query','metric','period','year'],additionalProperties:false},
    strict:true,
  },
  {
    ...
    parameters:{type:'object',properties:{queries:{type:'array',items:{type:'string'},minItems:2,maxItems:8},metric:{type:'string',enum:['energyKwh']},period:{type:['string','null']},year:{type:['string','null']}},required:['queries','metric','period','year'],additionalProperties:false},
    strict:true,
  },
  {
...
const SYSTEM_PROMPT = `Eres el Asistente Ambiental oficial de SiMeCO₂, un sistema educativo de Medellín que analiza consumo de energía eléctrica, huella de carbono y gestión energética de instituciones educativas.
...
function deterministicAnswer(grounded,message=''){
  ...
  if(grounded.intent==='institution_ranking_status'){
    ...
    if(r.included) return `**${r.institution.name}** sí está incluida en el ranking de ${r.metric?.label?.toLowerCase()||'energía eléctrica'} y ocupa el **puesto ${r.position} de ${r.totalRanked}**, con **${fmt(r.value)} ${r.metric?.unit||''}** y **${fmt(r.co2eT)} t CO₂e** en el alcance consultado.`;
    return `**${r.institution.name} no aparece en el ranking de ${r.metric?.label?.toLowerCase()||'energía eléctrica'} porque ${String(r.reason||r.qualityLabel||'no tiene un dato válido para clasificar').toLowerCase()}**. Esto **no significa consumo cero**. Estado de calidad: **${r.qualityLabel||r.qualityStatus||'Datos parciales'}**.`;
  }
  if(grounded.intent==='ranking'){
    const r=grounded.ranking; if(!r) return null;
    const rows=(r.ranking||[]).slice(0,10).map(x=>`${x.position}. ${x.name}: **${fmt(x.value)} ${r.metric?.unit||''}** · **${fmt(x.co2eT)} t CO₂e**`).join('\n');
    const filter=r.filter?.period?` en ${periodName(r.filter.period)}`:r.filter?.year?` en ${r.filter.year}`:'';
    return `**Top ${Math.min(10,(r.ranking||[]).length)} de ${r.metric?.label||'energía eléctrica'}${filter}:**\n\n${rows}\n\nSe clasificaron ${fmt(r.totalRanked)} instituciones/sedes con dato disponible; ${fmt(r.totalExcluded)} quedaron fuera por ausencia o estado de calidad del dato.`;
  }
