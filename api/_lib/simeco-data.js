...
const DEFAULT_CO2_FACTOR_KG_KWH = 0.126;
...
function ranking(metric = 'energyKwh', opts = {}) {
  ...
  for (const item of institutions) {
    ...
    rows.push({
      ...
      name:item.displayName,
      value:s.value,
      co2eT:s.value*DEFAULT_CO2_FACTOR_KG_KWH/1000,
      periodCount:new Set(list.map(r=>r.period)).size,
      nuclei:item.nuclei,
      territories:item.territories,
...
function rankingStatusForInstitution(query, metric = 'energyKwh', opts = {}) {
  ...
  return {
    institution:{key:item.key,name:item.displayName}, metric:{field:metric,...METRICS[metric]}, filter:{period,year},
    included:index>=0, position:index>=0?index+1:null, value:s.value,
    co2eT:s.value*DEFAULT_CO2_FACTOR_KG_KWH/1000,
...
function cityIndicators(opts = {}) {
  ...
  const totals = totalsFor(list);
  const official = summaries.filter(s => {
    if(opts.period) return s.period===opts.period;
    if(opts.year) return String(s.period||'').startsWith(String(opts.year));
    return true;
  });
  const officialEnergyValues=official
    .map(s=>nullableNumber(s.energyKwh))
    .filter(v=>v!==null);
  ...
  return {
    ...
    totals,
    officialSummaries: official.map(s => ({
      period:s.period,
      source:s.source,
